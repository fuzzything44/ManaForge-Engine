...
using meter_per_second_squared	
						= boost::mpl::vector<acceleration,  std::ratio<1> >;
...
public:
	...
	template<typename DimensionsU,
		typename = std::enable_if<
			boost::mpl::equal<
				...
				boost::mpl::at_c<Dimensions,  0>::type
			>::type::value
		>
	>
	...
	template<typename DimensionsU,
		...
		>
	>
	const Quantity& operator=(const quantity<DimensionsU>& other)
	{
		constexpr float multiplier =
	...
	{
		Quantity < boost::mpl::vector<
			boost::mpl::at_c<DimensionsU, 0>::type,
			boost::mpl::at_c<Dimensions, 1>::type
		>> otherConverted = other;


		using outputDims = boost::mpl::transform<
			boost::mpl::at_c<Dimensions,  0>::type,
			boost::mpl::at_c<DimensionsU, 0>::type,
